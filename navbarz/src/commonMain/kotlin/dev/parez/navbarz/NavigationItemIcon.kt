 ...
 *
 * Failing loudly on a missing icon is deliberate. An item with no icon at all would otherwise
 * render as an invisible tap target, which is far harder to diagnose than an exception naming the
 * item.
 */
@Composable
internal fun NavigationItemIcon(
    ...
    val badge: (@Composable () -> Unit)? =
        when {
            !showBadge -> null
            item.badge != null -> ({
                    ItemBadge(badgeContainerColor, badgeContentColor) { Text(item.badge) }
                })
            item.showBadgeDot -> ({
                    ItemBadge(badgeContainerColor, badgeContentColor, content = null)
                })
            else -> null
        }
