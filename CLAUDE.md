...
**NavBarz** — a published Compose Multiplatform SDK providing adaptive navigation chrome
(bottom bar / rail / drawer), with native `UITabBarController` chrome on iOS, plus a sample app.

```
:navbarz              the published SDK. android, jvm, js, wasmJs, iosArm64, iosSimulatorArm64
...
:sample:desktopApp    )
:sample:webApp        )
sample/iosApp/        Xcode project; roots itself in navBarzTabBarController, not DemoApp()
```

Coordinates: `dev.parez.navbarz:navbarz`. Root project is `navbarz-sdk`, **not** `navbarz` — a
root and a module with the same name collide in type-safe project accessors.

## Commands

```sh
./gradlew :navbarz:allTests                    # jvm + androidHostTest + iosSimulatorArm64
./gradlew :navbarz:publishToMavenLocal         # verifies the whole publishing setup
./gradlew :navbarz:checkKotlinAbi              # fails if the public API changed
./gradlew :navbarz:updateKotlinAbi             # accept an intentional API change
...
Single test: `./gradlew :navbarz:jvmTest --tests "*NavigationModeResolverTest.landscape*"`
...
`KotlinMultiplatformAndroidLibraryTarget`, which it skips. Hence `navbarz/api/jvm/` and no
`navbarz/api/android/`. There is no androidMain-only public API today; if you add some, it ships
unguarded.
...
**NavBarz builds the three navigation components itself**, delegating only *placement* to
...
**NavBarz has no FAB, deliberately.** A floating action button is an Android idiom and the iOS
...
**iOS native chrome is Kotlin, not Swift.** `NavBarzTabBarController.kt` (iosMain) builds a real
...
**NavBarz is consumable from the Kotlin Toolchain (ex-Amper) as published, with no extra artifact.**
Verified against CLI 0.12.2: a `module.yaml` with `dependencies: [dev.parez.navbarz:navbarz:…]`
compiles on jvm, android, iosArm64, iosSimulatorArm64, js and wasmJs. Its resolver reads Gradle
Module Metadata, so the root coordinate fans out to the per-platform variants the way Gradle does —
there is nothing Gradle-specific to publish alongside. The toolchain is still Alpha and cannot host
`:navbarz` itself (no ABI-validation equivalent to `abiValidation {}`, which is a Kotlin *Gradle*
...
three tabs, deliberately — NavBarz is what is being shown off, so it gets a third destination.
...
drawn by the scaffold *outside* `content`, so it stays sharp and becomes the only thing in focus.
Haze (`dev.chrisbanes.haze`) is a **sample** dependency; `:navbarz` does not depend on it and should
not. It makes the demo deliberately unreadable, so Settings has a **Blur Content** switch that turns
it off — reach for that rather than editing the modifier.
