...
import dev.parez.navbarz.NavigationMode
import dev.parez.navbarz.rememberNavigationMode
import dev.parez.navbarz.sample.di.LibraryKoinContext
import dev.parez.navbarz.sample.navigation.BrowserHistoryEffect
import dev.parez.navbarz.sample.navigation.DemoSavedStateConfiguration
import dev.parez.navbarz.sample.navigation.PokemonListKey
import dev.parez.navbarz.sample.theme.AppTypography
import dev.parez.navbarz.sample.theme.appColorScheme
import dev.parez.navbarz.sample.ui.PokeballMark
import dev.parez.navbarz.sample.ui.SettingsScreen
import dev.parez.navbarz.sample.ui.TeamFullDialog
import dev.parez.navbarz.sample.ui.TeamScreen
...
/**
 * The whole demo, navigation chrome included — NavBarz's Compose container picks a bottom bar, a
 * rail or a drawer from the window size.
 *
 * iOS roots itself in `navBarzTabBarController` and calls [DemoTab] per tab instead, so the system
...
private fun TabContent(tab: Tab) {
    ...
    // navBarzTabBarController gives each tab its own ComposeUIViewController.
...
private fun RailHeader() {
    Row(
        // No horizontal padding: the rail centres this, and in the drawer NavBarz applies the
