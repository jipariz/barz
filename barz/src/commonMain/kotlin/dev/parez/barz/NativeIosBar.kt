 ...
 *
 * Note this gives native *rendering*, not native *placement*: an embedded `UITabBar` is laid out
 * by Compose, so the system cannot relocate it the way it relocates a `UITabBarController`'s bar
 * on a foldable. For that, root the app in [barzTabBarController] instead.
 */
